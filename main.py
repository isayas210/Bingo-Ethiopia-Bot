...
def home():
    ...
    <head>
        ...
        <style>
            ...
            /* Selection Grid */
            .select-grid { display: grid; grid-template-columns: repeat(10, 1fr); gap: 4px; padding: 10px; background: #1a1a1a; border-radius: 10px; }
            ...
            .ball { font-size: 32px; color: #ffeb3b; border: 4px solid #0088cc; border-radius: 50%; width: 85px; height: 85px; margin: 15px auto; display: flex; align-items: center; justify-content: center; background: #000; box-shadow: 0 0 15px #0088cc; }
            ...
            .bingo-header { display: flex; justify-content: space-around; color: #0088cc; font-weight: 900; font-size: 0.85rem; padding-bottom: 4px; border-bottom: 1px solid #222; }
            ...
            .win-card { background: #1a1a1a; padding: 30px; border-radius: 20px; border: 2px solid #ffeb3b; }
    ...
    <body>
        <div id="win-overlay">
            <div class="win-card">
                ...
                <h2 style="color:#4caf50;">💰 +<span id="win-cash">0</span> ETB</h2>
        ...
        <div id="select-view">
            ...
            <div class="select-grid" id="s-grid"></div>
        </div>
        ...
        <script>
            ...
            let balance = 500.00;
            let tickets = [];
            let others = [3, 15, 22, 56, 89]; // Kelloo ta'uuf
            let pool = 0;
            ...
            for(let i=1; i<=100; i++) {
                ...
                if(others.includes(i)) d.classList.add('taken');
                    ...
                    d.onclick = () => {
                        if(tickets.length < 8 || d.classList.contains('selected')) {
                            if(!d.classList.contains('selected') && balance >= 10) {
                                d.classList.add('selected'); tickets.push(i); balance -= 10; pool += 10;
                            } else if(d.classList.contains('selected')) {
                                d.classList.remove('selected'); tickets = tickets.filter(x => x !== i); balance += 10; pool -= 10;
                            }
            ...
            function start() {
                if(tickets.length === 0) { location.reload(); return; }
            ...
            function play() {
                ...
                const loop = setInterval(() => {
                    if(isFinished) { clearInterval(loop); return; }
            ...
            function check(g) {
                ...
                if(won && !isFinished) {
                    isFinished = true; let prize = pool * 0.7; balance += prize;
                    document.getElementById('win-overlay').style.display = 'flex';
                    document.getElementById('win-id').innerText = g.id.replace('t-','');
                    document.getElementById('win-cash').innerText = prize.toFixed(2);
                }
            }
        </script>
    </body>
