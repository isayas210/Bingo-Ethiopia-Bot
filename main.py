...
@app.route('/')
def home():
    # UI'n kee akkuma jirutti itti fufa (Lakkofsa 1-100 fi Bingo Board)
    return """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .selection-grid { display: grid; grid-template-columns: repeat(10, 1fr); gap: 4px; padding: 10px; }
            .t-num { aspect-ratio: 1; background: #333; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; border-radius: 4px; }
            .t-num.selected { background: #4caf50 !important; border: 1px solid white; }
            #game-view { display: none; }
            .ball { font-size: 45px; color: #ffeb3b; border: 5px solid #0088cc; border-radius: 50%; width: 100px; height: 100px; margin: 15px auto; display: flex; align-items: center; justify-content: center; background: #000; }
            .cards-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
            .card { background: #151515; border: 1px solid #333; border-radius: 10px; padding: 8px; }
            .b-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 2px; }
            .cell { background: #262626; aspect-ratio: 1; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; }
    ...
    <body>
        ...
        <script>
            let balance = localStorage.getItem('bingo_v17_bal') ? parseFloat(localStorage.getItem('bingo_v17_bal')) : 500.00;
            function updateUI() { document.getElementById('bal-text').innerText = balance.toFixed(2); localStorage.setItem('bingo_v17_bal', balance); }
            updateUI();
            ...
            setInterval(() => {
                ...
                if(remaining === 59) { 
                    if(myTickets.length > 0) { isPlayingThisRound = true; startLiveGame(); } 
                    else {
                        isPlayingThisRound = false;
                        document.getElementById('select-view').style.display = 'none';
                        document.getElementById('game-view').style.display = 'block';
                        document.getElementById('game-status').innerText = "👀 TAAJJABDII QOFA";
                        runBalls();
                    }
                }
            }, 1000);
            ...
            function startLiveGame() {
                ...
                myTickets.forEach(id => {
                    ...
                    div.innerHTML = `<div class="b-grid" id="g-${id}"></div><div style="font-size:0.6rem;">#${id}</div>`;
                    ...
                    card.nums.forEach((n, idx) => {
                        ...
                        c.innerText = (idx === 12) ? "FREE" : n; if(idx === 12) c.classList.add('marked');
                ...
                const loop = setInterval(() => {
                    ...
                    if(isPlayingThisRound) {
                        allCards.forEach(card => {
                            ...
                            if(checkBingo(card.marks)) { isDone = true; balance += 700; updateUI(); document.getElementById('win-overlay').style.display = 'flex'; }
                        });
                    }
                }, 3000);
            }
...
if __name__ == "__main__":
    # Server (Flask) jalqaba itti eegala
    t = Thread(target=run)
    t.daemon = True
    t.start()
    
    # 🚨 FIX: Konfliktii dhabamsiisuuf sekondii 15 eeggata (Render inni moofaan akka dhaabbatuuf)
    print("Waiting for old instance to spin down...")
    time.sleep(15) 

    while True:
        try:
            bot.remove_webhook()
            # 🚨 FIX: Interval=5 fi timeout=30 conflict hir'isuuf gargaara
